function [shortestPath,shortestPathLength] = re_onewaypath(binaryImage,x,y,dmat,idxx,L_range,Rangeconstant,start_idx)
alpha =1.5;
...
%   end     
% end
prev=find(idxx==start_idx(1));
start=find(idxx==start_idx(2));

if (isempty(prev) || isempty(start))
    prev = 1;
    start = 2;
else
    prev = prev(1);
...
%   pause;
 ...
 while(isempty(find(bool_unique_all_vertex==0))==0)     
     if(totalcount == 1)
        inextCity = start;       
        iminDist = dmat(prev,start);
        p_count = 2;      
        nextCity_debug = inextCity;
        path(end+1,1) = inextCity;
        distanceTraveled = distanceTraveled +...
        dmat(currentCity,inextCity);
         distancesNew(currentCity,:) =   distancesNew(currentCity,:)*alpha;
%          distancesNew(:,currentCity) =   distancesNew(:,currentCity)*alpha;
        ...
        distanceTraveled = distanceTraveled +...
        dmat(currentCity,inextCity);
         distancesNew(currentCity,inextCity) =   distancesNew(currentCity,inextCity)*alpha;
         distancesNew(inextCity,currentCity) =   distancesNew(inextCity,currentCity)*alpha;
           distancesNew(currentCity,:) =   distancesNew(currentCity,:)*alpha;
         distancesNew(:,currentCity) =   distancesNew(:,currentCity)*alpha;
