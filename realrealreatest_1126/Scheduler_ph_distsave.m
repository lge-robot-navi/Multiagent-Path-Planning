function [coordix, coordiy, coorditheta] = Scheduler_ph_distsave(robots)
clc;
clear all;
...
fprintf('Parameter load completed\n');

load('Parameter_dist','num_of_nodes','unique_all_vertex','dmat');
fprintf('Parameter_dist load completed\n');
...
load('modified_dist','dmat');
unique_all_vertex = unique_all_vertex(1:num_of_nodes,:);
fprintf('modified_dist load completed\n');

%function [shortestPath,shortestPathLength]=NNTSP(dmat,unique_all_vertex,Debug_Plot)
dmat
unique_all_vertex
[shortestPath,shortestPathlength] = NNTSP(dmat,unique_all_vertex);
...
fprintf('Node Assignment completed\n');
 ...
 %% 영역 재보정
 while((max(size_coordi) - min(size_coordi)) > min(size_coordi)/Divide_factor)     
...
for jj = 1:size(coordix,2)
        %% 0807변경
         [shortestPath] = onewaypath(binaryImage,coordix{jj}(:),coordiy{jj}(:),dmat,idxx{jj}(:),L_range,Rangeconstant,1.5); 
...
save('FinalResults','unique_all_vertex','dmat','ccoordix','ccoordiy','idx');
    ...
    for i=1:size(coordix,2)
          for j = 1:size(coordix{i},2)
             ...
             posy{i}(1,j) = 0.05*(1868 - coordix{i}(1,j));
             if(j ==size(coordix{i},2) )
                 posTheta{i}(1,j) = atan2(posy{i}(1,1)-posy{i}(1,j),posx{i}(1,1)-posx{i}(1,j));
             else
                 posTheta{i}(1,j) = atan2(0.05*(1868 - coordix{i}(1,j+1))-posy{i}(1,j),0.05*(coordiy{i}(1,j+1)-2180)-posx{i}(1,j));
             end
          end
    end
