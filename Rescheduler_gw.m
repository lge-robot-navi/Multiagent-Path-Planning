...
if(size(robots,1) == 1) % 초기 2대
    load('FinalResults_gw2','unique_all_vertex','dmat','ccoordix','ccoordiy','idx');
    load('Parameter_gw2','binaryImage','gx','gy','Rangeconstant','max_value','L_range','L_range_base','RGB','RGB2','POI','unique_all_vertex_base');
elseif(size(robots,1) == 2) % 초기 3대
    load('FinalResults_gw3','unique_all_vertex','dmat','ccoordix','ccoordiy','idx');
    load('Parameter_gw3','binaryImage','gx','gy','Rangeconstant','max_value','L_range','L_range_base','RGB','RGB2','POI','unique_all_vertex_base');
elseif(size(robots,1) == 3) % 초기 4대
    load('FinalResults_gw4','unique_all_vertex','dmat','ccoordix','ccoordiy','idx');
    load('Parameter_gw4','binaryImage','gx','gy','Rangeconstant','max_value','L_range','L_range_base','RGB','RGB2','POI','unique_all_vertex_base');
elseif(size(robots,1) == 4) % 초기 5대
    load('FinalResults_gw5','unique_all_vertex','dmat','ccoordix','ccoordiy','idx');
    load('Parameter_gw5','binaryImage','gx','gy','Rangeconstant','max_value','L_range','L_range_base','RGB','RGB2','POI','unique_all_vertex_base');
elseif(size(robots,1) == 5) % 초기 6대
    load('FinalResults_gw6','unique_all_vertex','dmat','ccoordix','ccoordiy','idx');
    load('Parameter_gw6','binaryImage','gx','gy','Rangeconstant','max_value','L_range','L_range_base','RGB','RGB2','POI','unique_all_vertex_base');
end

% save('kumohResults5','coordix','coordiy');
% pause;
%% 광운대학교 파트
% [coordix, coordiy, coorditheta, ~] = Func_fovPathPlanning('map_outline.jpg', ccoordix, ccoordiy); 
% 
% pause;
...
% plot(unique_all_vertex_base(:,2),unique_all_vertex_base(:,1),'or');
% pause;
...
%% 이전 waypoint, 현재 waypoint index 찾기
 if(check_debug)
    ...
    for i = 1:num_of_robots
          [tndist,tnidx] = sort(sqrt((wx_to_xi_gj(robots(i).curTargetx) -  unique_all_vertex(:,1)).^2+(wy_to_yi_gj(robots(i).curTargety) -  unique_all_vertex(:,2)).^2)); 
          [tndist1,tnidx1] =  sort(sqrt((wx_to_xi_gj(robots(i).prevTargetx) -  unique_all_vertex(:,1)).^2+(wy_to_yi_gj(robots(i).prevTargety) -  unique_all_vertex(:,2)).^2)); 
...
[coordix, coordiy, coorditheta, ~] = Func_fovPathPlanning('map_gw.bmp', coordix, coordiy); % 1:출력, 2:생략
