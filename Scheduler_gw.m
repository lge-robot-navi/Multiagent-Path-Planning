function [coordix, coordiy, coorditheta] = Scheduler_gw(robots)
clc;
clear all;
...
save('Test_save','dmat','unique_all_vertex','patrol_distance','binaryImage','gx','gy','Rangeconstant','max_value','L_range','L_range_base','RGB','RGB2','POI','unique_all_vertex_base');
fprintf('Test_save\n');
pause;
     ...
     count = 1;       
        total_node = 0;      
         for i=1:size(coordiy,2)
          ...
          itr = size(coordiy,2);       
           min_length = floor(total_node/(4*D_RobotNum)); %% 최소 개수 
...
fprintf('Kmeans_based_Clustering\n');
 ...
 while((max(size_coordi) - min(size_coordi)) > min(size_coordi)/6)    
     ...
     maxidxb = randsample(maxidxb(1:3),3);
...
save('FinalResults','unique_all_vertex','dmat','ccoordix','ccoordiy','idx');
    ...
    for i=1:size(coordix,2)
          for j = 1:size(coordix{i},2)
             posx{i}(1,j) = 0.05*(  coordiy{i}(1,j)-(157.000006/0.05));
             posy{i}(1,j) = 0.05*( (175.349997/0.05) - coordix{i}(1,j));
             if(j ==size(coordix{i},2) )
                 posTheta{i}(1,j) = atan2(posy{i}(1,1)-posy{i}(1,j),posx{i}(1,1)-posx{i}(1,j));
             else
                 posTheta{i}(1,j) = atan2(0.05*((175.349997/0.05) - coordix{i}(1,j+1))-posy{i}(1,j),0.05*(coordiy{i}(1,j+1)-(157.000006/0.05))-posx{i}(1,j));
             end
          end
    end
